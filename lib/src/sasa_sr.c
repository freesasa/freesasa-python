/* sasa_sr.c — Shrake-Rupley SASA with OpenMP + AVX-512/AVX2 SIMD
 *
 * Parallelism layers:
 *   1. Atom-level: #pragma omp parallel for over atoms (existing)
 *   2. SIMD inner loop: neighbor distance checks vectorized with AVX-512 floats
 *      (16-wide) or AVX2 (8-wide) or auto-vectorization fallback
 *
 * Key data-layout change: per-atom SoA neighbor cache (float32)
 *   nb_cx[k], nb_cy[k], nb_cz[k], nb_r2[k] for k=0..nni-1
 * This gives 2x SIMD density vs double and sequential memory access.
 */
 ...
 * Layout: arrays of length nb->nn[i], one set per atom
 * Stored as flat float arrays in a single allocation per thread
 * ────────────────────────────────────────────────────────────────────────── */
typedef struct {
...
sr_build_nb_cache(sr_data *sr)
{
    ...
    for (int i = 0; i < n_atoms; ++i) {
        ...
        float *buf = (float *)aligned_alloc(64, sizeof(float) * padded * 4);
...
sr_free_nb_cache(sr_data *sr)
{
    ...
    for (int i = 0; i < sr->n_atoms; ++i) {
        if (sr->nb_cache[i].x) free(sr->nb_cache[i].x);
    }
    free(sr->nb_cache);
    sr->nb_cache = NULL;
}
